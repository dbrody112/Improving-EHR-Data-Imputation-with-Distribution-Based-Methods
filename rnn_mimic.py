...
def return_data(synth_data=False, balancer=True, target='MI', 
                ...
                time_steps=14, split=True, pad=True, mode = 'Okay'):
...
def build_model(no_feature_cols=None, time_steps=7, output_summary=False, lr = 0.001):
...
def train(model_name="kaji_mach_0", synth_data=False, target='MI',
          ...
          n_percentage=1.0, time_steps=14, epochs=10, use_pretrained = False,
         pretrained_model = None, lr = 0.001):
  ...
  ROOT_DIR = os.path.dirname(os.path.abspath('rnn_mimic.py'))
  log_dir = "logs"
  ending = f'{model_name}.log'
  
  training_log_dir_path = Path((os.path.join(ROOT_DIR, log_dir, ending)))
  training_log_dir = str(training_log_dir_path)
  #init callbacks
  tb_callback = TensorBoard(training_log_dir,
    histogram_freq=0,
    write_grads=False,
    write_images=True,
    write_graph=True) 
...
def return_loaded_model(model_name="kaji_mach_0", method_of_choice = "median imputation"):

  loaded_model = load_model("./experiments/{0}_FILES/saved_models/{1}.h5".format(method_of_choice,model_name))

  return loaded_model

def pickle_objects(target='MI', time_steps=14, mode = 'Okay'):
