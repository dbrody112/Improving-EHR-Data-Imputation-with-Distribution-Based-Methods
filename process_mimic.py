...
class ParseItemID(object):
    ...
    def __init__(self):
        ...
        self.feature_names = ['RBCs', 'WBCs', 'platelets', 'hemoglobin', 'hemocrit', 
                              ...
                              'daily weight', 'tobacco', 'diabetes', 'history of CV events', 'EKG', 'Chest Pain']
    ...
    def extractor(self, feature_name, pattern):
        ...
        if(feature_name == 'EKG' or feature_name == 'Chest Pain'):
...
class MimicParser(object):
    ...
    def create_day_blocks(self, file_name):
        ...
        print(df3.columns)
        df3.columns = ["{0}_std".format(i) for i in list(df3.columns)]
        ...
        for i in list(df2.columns):
            df2[i][df2[i] > df2[i].quantile(.95)] = np.nan
        
        percent_missing = (df2.isnull().sum() *100) / len(df)
        missing_value_df = pd.DataFrame({'column_name' : df2.columns, 'percent_missing' : percent_missing})
        print(missing_value_df)
        
        sns.displot(missing_value_df, x = "percent_missing")
        plt.legend()
        
        missing_value_df.to_csv("missing_rate_by_col.csv")
        
        df2.to_csv("orig_chartevents_data.csv")
        
        plt.savefig('missing_rates.jpg')
        
        
        
        sys.exit()
        
        def imputation_meth(df2, mode):
            for i in list(df2.columns):
                if(mode == "median imputation"):
                    df2[i][df2[i] > df2[i].quantile(.95)] = df2[i].median()
                    df2[i].fillna(df2[i].median(), inplace=True)
                elif(mode == "mean imputation"):
                    df2[i][df2[i] > df2[i].quantile(.95)] = df2[i].mean()
                    df2[i].fillna(df2[i].median(), inplace=True)
                elif(mode == "GAIN" or "MIM-GAIN"):
                    #add hyperparameter searchw/ auroc
                    df2[i][df2[i] > df2[i].quantile(.95)] = np.nan
                    df2[i].fillna(np.nan, inplace=True)
                    method = 'Normal'
                    if mode == "MIM-GAIN": 
                        method = 'MIM'
                    gain_parameters = {'batch_size': 1000,
                                               'hint_rate': 0.9,
                                               'alpha': 100,
                                               'iterations': 10000,
                                               'mode' : method}
                    imputed_data_x = gain(np.array(df2.values), gain_parameters)
            
                    df2 = pd.DataFrame(imputed_data_x, columns = df2.columns, index = df2.index)
...
#                df2[i] = df2[i].where(df2[i] > df2[i].quantile(.875)).fillna(df2[i].median())
    ...
    def add_icd_infect(self, file_name):
        ...
        #add chest pain and instead use nans
